...
if (!$tenant_id || !$listing_id || !$start_date || !$end_date || !$contact_name || !$contact_phone || !$contact_email) {
    die('Missing fields');
}
...
try {
  ...
  $ins = $pdo->prepare("
    ...
    (tenant_id, listing_id, start_date, end_date, status, contact_name, contact_phone, contact_email)
    VALUES 
    (:tenant, :listing, :start, :end, 'confirmed', :cname, :cphone, :cemail)
");
$ins->execute([
    ':tenant'  => $tenant_id,
    ':listing' => $listing_id,
